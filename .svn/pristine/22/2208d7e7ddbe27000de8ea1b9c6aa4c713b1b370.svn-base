<template>
  ...
  <div class="mod-config">
    ...
    <add-or-update @refreshDataList="getDataList" ref="addOrUpdate" v-if="addOrUpdateVisible"
                   v-bind:doctorCase="dataCase"></add-or-update>
  </div>
</template>

<script>
  import AddOrUpdate from './ExpertDetapopup'
  ...
  export default {
    ...
    activated () {
      this.exaIdNum = this.$route.query.userId
    ...
    methods: {
      downloadFile () {//导出信息
        ...
        this.searchData.teacherUserId = this.exaIdNum
      ...
      search (pageIndex = 1) {//搜索
        ...
        this.searchData.page = pageIndex,
          // this.searchData.relationUserId = this.exaIdNum
