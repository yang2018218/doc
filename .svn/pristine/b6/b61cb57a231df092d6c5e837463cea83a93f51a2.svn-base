<template>
  <el-dialog
    ...
    <el-form-item label="字典表id逗号分隔" prop="units">
      <el-input v-model="dataForm.units" placeholder="字典表id逗号分隔"></el-input>
    </el-form-item>
    ...
    data () {
      return {
        ...
        dataRule: {
          ...
          units: [
            { required: true, message: '字典表id逗号分隔不能为空', trigger: 'blur' }
          ],
          mainWarningLine: [
    ...
    methods: {
      init (id) {
        ...
        this.$nextTick(() => {
          ...
          if (this.dataForm.modelId) {
            this.$http({
              url: this.$http.adornUrl(`/ycyl/ycylmainproductmodel/info/${this.dataForm.modelId}`),
              method: 'get',
      ...
      dataFormSubmit () {
        this.$refs['dataForm'].validate((valid) => {
          if (valid) {
            this.$http({
              url: this.$http.adornUrl(`/ycyl/ycylmainproductmodel/${!this.dataForm.modelId ? 'save' : 'update'}`),
              method: 'post',
