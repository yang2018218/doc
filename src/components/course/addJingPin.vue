<template>
  <div class="capterBox">
    ...
    <section class="openBox">
      是否公开：
      <el-select v-model="theOpenMessage" placeholder="请选择" @change="openFunction(theOpenMessage)">
        <el-option
          v-for="item in opens"
          :key="item.value"
          :label="item.label"
      ...
      <label v-if="tryVideo">试学时间：
        <el-input
          @input="tcn"
          placeholder="单行输入"
          v-model.number="dataOrigin.tryTime"
          clearable>
        </el-input>
      </label>
    </section>
    ...
    <section v-if="videoPlayBox" class="videoPlayBoxstyle">
      <div class="prism-player">
        <el-button type="danger" v-if="videoPlayBox" class="removeVideoBtn" style="padding:8px 15px;z-index: 9999"
                   @click="removeVideoFun()">移除
        </el-button>
        <video :src="videoUrlStady" controls="controls"></video>
      </div>
    </section>
    <!--    <section v-if="testStudyBox" class="testVideoStyle">-->
    <!--      <p>试学视频</p>-->
    <!--      <test-study :messageTitleName="dataOrigin.chapterName" @studyVideoId='studyVideo' ref="testStudyCom"-->
    <!--                  :testVideoId="testVideoId"></test-study>-->
    <!--    </section>-->
    <section>
      视频排序：
      <el-input
...
<script>
  ...
  export default {
    ...
    methods: {
      tcn () {
        if (Number(this.dataOrigin.tryTime)) {

        } else {
          this.dataOrigin.tryTime = ''
      ...
      openFunction (val = '0') {
        if (val == '1') {
          this.tryVideo = false
          this.dataOrigin.tryTime = "0"
        } else {
          this.tryVideo = true
        }
        this.dataOrigin.theOpen = val + ''
      },
      removeVideoFun () {
      ...
      initBianJi (id) {
        this.drugPopupList = []
        this.theOpenMessage = '0'
        this.tryVideo = true
        this.videoPlayBox = false
        this.videoUUpload = true
        // this.testStudyBox = false
        this.authProgress = 0
        this.DrugName = ''
        this.dataOrigin = {
          theme: '',//推荐主题
        ...
        if (id) {
          ...
          this.$http({
            ...
            if (data && data.code === 0) {
              ...
              if (data.chapter.attemptFileId) {
                this.testVideoId = data.chapter.attemptFileId
                this.$nextTick(() => {
                  this.$refs.testStudyCom.init(this.testVideoId)
                })
              } else {
              }
              this.wordTable = data.product
              this.theOpenMessage = data.chapter.theOpen + ''
              this.getVideoUrl(data.chapter.fileId)
              if (this.dataOrigin.theOpen == 1) {
                this.tryVideo = false
              } else {
                this.tryVideo = true
              }
      ...
      addStudycurriculumchapterBianJi (id) {//编辑
        ...
        if (this.drugPopupList.length > 1 && this.dataOrigin.theme && this.dataOrigin.chapterName && this.dataOrigin.fileId && this.dataOrigin.duration && this.dataOrigin.orderBy) {
          ...
          if (this.dataOrigin.theOpen == '0' && !this.dataOrigin.tryTime) {
            this.$message.error('温馨提示：请输入试看时间')
          } else {
            this.$http({
