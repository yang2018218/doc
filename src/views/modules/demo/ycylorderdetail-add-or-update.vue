    ...
    methods: {
      init (id) {
        ...
        this.$nextTick(() => {
          ...
          if (this.dataForm.detailId) {
            this.$http({
              url: this.$http.adornUrl(`/generator/ycylorderdetail/info/${this.dataForm.detailId}`),
              method: 'get',
              params: this.$http.adornParams()
            }).then(({data}) => {
              if (data && data.code === 0) {
                this.dataForm.orderId = data.ycylorderdetail.orderId
                this.dataForm.productId = data.ycylorderdetail.productId
                this.dataForm.modelId = data.ycylorderdetail.modelId
                this.dataForm.modelName = data.ycylorderdetail.modelName
                this.dataForm.price = data.ycylorderdetail.price
                this.dataForm.marketPrice = data.ycylorderdetail.marketPrice
                this.dataForm.count = data.ycylorderdetail.count
                this.dataForm.units = data.ycylorderdetail.units
                this.dataForm.createTime = data.ycylorderdetail.createTime
              }
            })
          }
        })
      ...
      dataFormSubmit () {
        this.$refs['dataForm'].validate((valid) => {
          if (valid) {
            this.$http({
              url: this.$http.adornUrl(`/generator/ycylorderdetail/${!this.dataForm.detailId ? 'save' : 'update'}`),
              method: 'post',
