  ...
  <div id="addMianfei">
    <div class="content">
      ...
      <el-form :inline="true" :model="course">
        ...
        <el-form-item label="会员是否免费：">
          <el-select
            style="display:inline-block"
            v-model="course.memberTheFree"
            clearable
            placeholder="请选择"
            label-width="auto"
          >
            <el-option v-for="(item,index) in memberFree" :key="index" :label="item.name" :value="item.val"></el-option>
          </el-select>
        </el-form-item>
        <el-form-item
          label="课程价格："
      ...
      <footer class="footerSty">
        <div>
          <UEditor ueditorId="ueditor2" :config="config" ref="ueditor"></UEditor>
        </div>
      </footer>
      <div class="btnStyle">
  ...
  export default {
    ...
    methods: {
      ...
      preserveMessage () {
        ...
        console.log(this.course.fitKind)
        if (
          ...
          this.$http({
            ...
            if (data && data.code === 0) {
              ...
              this.$router.push({path: 'course-goodsclass'})
            } else {
